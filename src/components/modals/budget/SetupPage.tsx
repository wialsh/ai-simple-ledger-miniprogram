...
import { TopBarModal } from '@/components/bar/TopBar';
import { AppContext } from '@/context/AppContext';
import { Icon } from '@/components/ui/Icon';
import { YeayTimerModal } from '@/components/ui/timer/Timer';
import { Dialog } from '@/components/ui/Dialog';
import type { Budget } from '@/types';
import { InputItem } from './InputItem';

interface BudgetSetupPageProps {
  onClose: () => void;
}
...
export const BudgetSetupPage: React.FC<BudgetSetupPageProps> = ({ onClose }) => {
  const { currentDate, currentLedger, updateLedgerBudget } = useContext(AppContext);
  ...
  // 初始化数据
  useEffect(() => {
    const findedBudget = currentLedger?.budgets?.find(b => b.year === planDate.getFullYear());
    const newValues = new Array(12).fill('');
    if (findedBudget) {
      findedBudget.amounts.forEach((amount, i) => {
        if (i < 12) {
  ...
  useEffect(() => {
    ...
    if (budget) {
      updateLedgerBudget(budget);
    }
  }, [budget, updateLedgerBudget]);
    ...
    <RootPortal>
      ...
      >
        ...
        <TopBarModal>
          ...
          >
            <YeayTimerModal date={planDate} setDate={setPlanDate} />
            <View
